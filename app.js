...
document.addEventListener("DOMContentLoaded", ()=>{
  ...
  document.querySelectorAll("#menu button[data-go]").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      ...
      if(cible){ document.querySelectorAll("section").forEach(s=>s.style.display="none"); cible.style.display="block"; menu.classList.remove("show"); }
    });
  });
  ...
  function updatePanierDOM(){
    ...
    panier.forEach((p,i)=>{
      ...
      div.querySelector("button").addEventListener("click",()=>{ panier.splice(i,1); updatePanierDOM(); });
    ...
    if(compteurPanier) compteurPanier.innerText=panier.length;
  }
  ...
  function afficherProduits(){
    ...
    produits.filter(p=>p.nom.toLowerCase().includes(recherche)).forEach(p=>{
      ...
      div.querySelector("button").addEventListener("click",()=>{ panier.push(p); updatePanierDOM(); });
  ...
  chatSend.addEventListener("click", async ()=>{
    ...
    try{
      ...
      divIA.innerHTML=`<b>Assistant IA:</b> ${data.reply}`;
  ...
  document.getElementById("envoyerMessage").addEventListener("click", async ()=>{
    ...
    ok=document.getElementById("contactOk");
    ok.innerText="✅ Message envoyé !";
